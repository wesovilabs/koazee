...
func BenchmarkMapString10000ToUpper(b *testing.B) {
	b.StopTimer()
	var outStream stream.Stream
	for i := 0; i < b.N; i++ {
		stream := koazee.
...
func BenchmarkMapString10000ToIntLen(b *testing.B) {
	b.StopTimer()
	var outStream stream.Stream
	for i := 0; i < b.N; i++ {
		stream := koazee.
	...
	for index, val := range outStream.Out().Val().([]int) {
		if len(strings10000[index]) != val {
			b.Fatalf("Expected %d but retrieved %d in position %d", len(strings10000[index]), val, index)
		}
	}
}
