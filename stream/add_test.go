...
func TestStream_Add_validation(t *testing.T) {
	...
	s := koazee.
		...
		Add(10)
	assert.Equal(t, errors.InvalidArgument(stream.OpCodeAdd, "An element whose type is int can not be added in a stream of type string"), s.Out().Err())

	s = koazee.Stream().Add(10)
	assert.Equal(t, errors.ItemsNil(stream.OpCodeAdd, "An element can not be added in a nil stream"), s.Out().Err())

	s = koazee.Stream().Add(nil).With([]int{2, 3, 1})
	assert.Equal(t, errors.InvalidArgument(stream.OpCodeAdd, "A nil value can not be added in a stream of non-pointers values"), s.Out().Err())
}
