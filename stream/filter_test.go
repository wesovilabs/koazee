...
func TestStream_Filter_validation(t *testing.T) {
	assert.Equal(
		t,
		errors.InvalidArgument(stream.OpCodeFilter, "The filter operation requires a function as argument"),
	...
	assert.Equal(
		t,
		errors.InvalidArgument(stream.OpCodeFilter, "The provided function must retrieve 1 argument"),
		koazee.StreamOf([]int{2, 3, 2}).Filter(func() {}).Out().Err())

	assert.Equal(
		t,
		errors.InvalidArgument(stream.OpCodeFilter, "The type of the argument in the provided function must be int"),
		koazee.StreamOf([]int{2, 3, 2}).Filter(func(val string) bool { return true }).Out().Err())

	assert.Equal(
		t,
		errors.InvalidArgument(stream.OpCodeFilter, "The provided function must return 1 value"),
